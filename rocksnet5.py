...
class Player(Net):
    def setup(self):
        ...
        h0 = create_dense(screens, 256, 64)
        h1 = create_dense(h0, 64, 16)
        actions = create_dense(h1, 16, len(game.actions()))
...
def create_conv(input, in_channels, out_channels, weight_set=[]):
    patch_size = 3
    w = weight_var([patch_size, patch_size, in_channels, out_channels], stddev=0.001)
...
def backfill_rewards(experiences, reward_decay=0.7):
...
def train(player_net, explore_prob=0.1, evaluate=False):
    ...
    experiences = [exp for exp in experiences if exp[2] > 0.1]
    ...
    if len(training_tuples) > 10: training_tuples = training_tuples[:10]
...
def run():
    player_net = Player(dir_path='model/rocksnet5')
    step = 0
    if not show: print "Average reward per game, random-action baseline:", train(player_net, 1)
    while True:
        step += 1
