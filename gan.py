...
class GAN(Net):
    ...
    def setup(self):        
        ...
        def create_deconv(input, in_channels, out_channels, input_image_size, weight_set=[], no_bias=False):
        ...
        gen = create_deconv(gen, 8, 3, 32, gen_weights) # 64 x 64 x 3
        ...
        disc = tf.nn.dropout(disc, disc_dropout_keep_prob)
        disc = create_conv(disc, 16, 16, disc_weights) # now 4 x 4 x 16
    ...
    def train(self, inp, outp):
        ...
        # target_loss = 0.1 # 0.5
        train_gen = 0.5 # random.random() < 0.75 # self.last_loss is None or self.last_loss < target_loss
