...
bottleneck_size = 64 # 1319 emoji in dataset
...
class ImageAutoEncoder(Net):
    def setup(self):
        ...
        def create_deconv(input, in_channels, out_channels, image_size, out_dict={}):
            ...
            output_shape = tf.pack([batch_size, image_size*2, image_size*2, out_channels])
        ...
        self.save_last_deconv = {}
        img = create_deconv(img, 8, 3, 32, out_dict=self.save_last_deconv) # now img is 64 x 64, 3 channels
    ...
    def evaluate(self, inputs, outputs):
        ...
        print loss, mean, var
        # save last deconv:
        w, b = self.session.run([self.save_last_deconv['w'], self.save_last_deconv['b']])
        np.save('last_deconv.npy', np.array([w, b]))
        return loss
...
n = ImageAutoEncoder(dir_path='model/autoencoder2-64')
# n = ImageAutoEncoder()
